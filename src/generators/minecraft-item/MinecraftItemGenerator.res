...
let script = () => {
  Generator.defineCustomStringInput("CurrentTexture", (onChange: string => unit) => {
    <TexturePicker
      texture=MinecratItem_Texture_minecraft_1_18_1.texture
      frames={Generator_TextureFrame.texturesToFrames(
        MinecratItem_Texture_minecraft_1_18_1.tiles->Generator_TextureFrame.asTextures_UNSAFE,
        16,
      )}
      onSelect={frame => {
  ...
  | Some(frame) => {
      Generator.drawTexture(
        MinecratItem_Texture_minecraft_1_18_1.texture.id,
        frame.rectangle,
        (100, 100, 16 * 8, 16 * 8),
        (),
      )
